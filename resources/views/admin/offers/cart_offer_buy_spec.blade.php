...
<span class="pull-right">Total: {{$thetotalall}} L.L</span></div>
    ...
    @endforeach
     @foreach($carts_sub_item as $cartchild)
       ...
       <tr border: 1px solid #ddd!important>
        <td style="height:50px;width:50px; background: url('{{asset('uploads/items/childs'.$carts_sub_item->child->img_name)}}'); background-size: 100%; background-position: center center;border: 1px solid #ddd!important"></td>
